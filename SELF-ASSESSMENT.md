...
- Used all-MiniLM-L6-v2 sentence embeddings for semantic retrieval in RAG system. See `src/rag_system.py` lines 13-15 (model initialization) and lines 71-83 (`build_index()` method using FAISS similarity search).
...
- Engineered separate prompts for RAG mode (`config/prompts.md`) and No-RAG mode (`config/prompts_no_rag.md`). RAG prompts include specific instructions for SEC filing sections (e.g., "Focus on information from Section 1A Risk Factors"), while No-RAG prompts avoid filing-specific references. See README.md Evaluation section for qualitative comparison demonstrating RAG prompts produce more grounded, specific responses.

**Collected or constructed original dataset through substantial engineering effort with documented methodology (10 pts)** [Remove]
- Financial PhraseBank dataset (`data/Sentences_75Agree.txt`, 4,840 labeled sentences) preprocessed for DistilBERT fine-tuning. Data loading and preprocessing in `notebooks/sentiment_analysis_training.ipynb` cells 1-5 (lines 1-120), including train/validation/test splitting and tokenization. See ATTRIBUTION.md for dataset source.

**Fine-tuned pretrained model on your dataset (5 pts)**
- Fine-tuned DistilBERT on Financial PhraseBank for financial sentiment classification. Training implementation in `notebooks/sentiment_analysis_training.ipynb` cells 8-12 (training loop), with final validation accuracy of 93.05%. Model weights saved in `models/distillbert-fine-tuning/`.
...
- Training and validation loss/accuracy tracked over 3 epochs in `notebooks/sentiment_analysis_training.ipynb`. Visualizations include confusion matrix (`docs/figures/distillbert_confusion_matrix.png`) and ROC curves (`docs/figures/distillbert_roc_curve.png`) shown in README.md.
...
- Deployed as Streamlit web application (`app.py`) with interactive UI for company analysis, side-by-side RAG vs. No-RAG comparison, and sentiment analysis visualization. See SETUP.md for deployment instructions and README.md for features.
...
- README.md includes "What It Does" section, "Quick Start" section, "Video Links" section with placeholders, "Evaluation" section with quantitative and qualitative results, and "Individual Contributions" section.

**SETUP.md exists with clear, step-by-step installation instructions**
- SETUP.md provides comprehensive setup instructions (environment creation, dependency installation, API key configuration, and instructions for graders to test the system with and without Duke AI Gateway access).
...
- Video links section exists in README.md with placeholders for demo and technical walkthrough videos.

**Project workshop attendance**
- Attended all 6 project workshop days.

**Repository organization follows requirements**
- Repository includes required directories: `src/` (source code), `data/` (datasets and scripts), `models/` (trained models), `notebooks/` (Jupyter notebooks), `docs/` (documentation and figures), and `videos/` (placeholder for demo videos).
...
## Category 1: Machine Learning (Maximum 75 points, select at most 15 items)

**Solo project with one contributor (5 pts)**
- Solo project completed by Arnav Meduri. See "Individual Contributions" section in README.md and ATTRIBUTION.md.
...
**Built retrieval-augmented generation (RAG) system with document retrieval and generation components (10 pts)**
- Built RAG system retrieving from SEC EDGAR filings (10-K, 10-Q) with GPT-4.1 generation. Document retrieval in `src/rag_system.py` (FAISS indexing and semantic search), SEC filing integration in `src/clients/edgartools_client.py`, and generation in `src/clients/duke_gateway_model.py`. Full pipeline orchestrated in `src/finbrief.py` lines 1241-1550.
...
**Used pretrained model as frozen feature extractor with custom classifier head (5 pts)**
- DistilBERT base model used as feature extractor with custom 3-class classification head (Positive/Neutral/Negative). See `notebooks/sentiment_analysis_training.ipynb` cell 6 for model architecture definition with `num_labels=3` configuration.

**Modular code design with reusable functions and classes rather than monolithic scripts (3 pts)**
- Modular architecture with separate classes: `RAGSystem` (`src/rag_system.py`), `SentimentClassifier` (`src/sentiment_classifier.py`), `DukeGatewayModel` (`src/clients/duke_gateway_model.py`), `EdgarToolsClient` (`src/clients/edgartools_client.py`), `FinnhubClient` (`src/clients/finnhub_client.py`). Main orchestration in `FinBriefApp` class (`src/finbrief.py`). See project structure in README.md.
...
**Trained model using GPU/TPU/CUDA acceleration (5 pts)**
- DistilBERT fine-tuning performed on Google Colab with GPU acceleration (Tesla T4). Training environment documented in README.md and `notebooks/sentiment_analysis_training.ipynb`.
